...
import static me.earth.autothirtytwok.util.CheckUtil.holding32k;
import static me.earth.autothirtytwok.util.CheckUtil.is32k;
...
public class EarthAuto32k extends Module {

    private static final ModuleCache<Freecam> FREECAM =
            Caches.getModule(Freecam.class);

    private static final EarthAuto32k INSTANCE = new EarthAuto32k();

    public Setting<Mode> mode = register(new EnumSetting<>("Mode", Mode.NORMAL));
    ...
    public final Setting<Boolean> autoSwitch = register(new BooleanSetting("AutoSwitch", false))
        .setComplexity(Complexity.Medium);
    ...
    protected final Setting<Boolean> antiHopper = register(new BooleanSetting("AntiHopper", false))
        .setComplexity(Complexity.Medium);
    protected final Setting<Boolean> titleNotif = register(new BooleanSetting("TitleNotification", false))
            .setComplexity(Complexity.Medium);
    protected final Setting<Boolean> sphere = register(new BooleanSetting("Sphere", true))
            .setComplexity(Complexity.Medium);
    ...
    public EarthAuto32k() {
        super("EarthAuto32k", Category.Combat);
    ...
    public static EarthAuto32k getInstance() {
        return INSTANCE;
    }
        ...
        if (mc.player.openContainer instanceof ContainerHopper) {
            if (!holding32k(mc.player)) {
                ...
                if (closeGui.getValue() && secretClose.getValue()) {
                    ...
                    if (titleNotif.getValue())
                        mc.ingameGUI.displayTitle(TextColor.GREEN + "32k Equipped", "", 10, 100, 0);
                }
            }
